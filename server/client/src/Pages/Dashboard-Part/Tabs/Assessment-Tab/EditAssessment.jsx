...
const EditAssessment = forwardRef(({ onClose, assessmentId, candidate1 }, ref) => {
  ...
  useEffect(() => {
    const fetchAssessment = async () => {
      try {
        const response = await axios.get(`${process.env.REACT_APP_API_URL}/assessment/${assessmentId}`);
  ...
  const handleSaveAll = async (e) => {
    ...
    try {
      ...
      const response = await axios.put(`${process.env.REACT_APP_API_URL}/assessment/${assessmentId}`, updatedAssessment);
