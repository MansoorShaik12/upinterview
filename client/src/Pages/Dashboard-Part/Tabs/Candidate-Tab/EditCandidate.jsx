...
const CreateCandidate = ({ isOpen, onClose, handleOutsideClick, candidate1 }) => {
  ...
  const handleSubmit = async (_id, e) => {
    ...
    try {
      const response = await axios.put(`http://localhost:3000/candidate/${_id}`, data);
      ...
      if (file) {
        ...
        try {
          await axios.delete(`http://localhost:3000/candidate/${candidateId}/image`);
        } catch (error) {
  ...
  useEffect(() => {
    const fetchCollegeData = async () => {
      try {
        const response = await axios.get('http://localhost:3000/universitycollege');
            ...
            <div className="grid grid-cols-4">
              <div className="col-span-3">
                    ...
                    {showDropdownQualification && (
                      <div className="absolute z-50 -mt-3 mb-5 w-full rounded-md bg-white shadow-lg">
                        {qualification.map((qualification) => (
                          <div key={qualification.code} className="py-2 px-4 cursor-pointer hover:bg-gray-100" onClick={() => handleQualificationSelect(qualification)}>
                ...
                <div className="flex gap-5 mb-5">
                  ...
                  <div className="relative flex-grow">
                    ...
                    {showDropdownCollege && (
                      <div className="absolute z-50 -mt-3 mb-5 w-full rounded-md bg-white shadow-lg">
                        {college.map((college) => (
                          <div key={college.code} className="py-2 px-4 cursor-pointer hover:bg-gray-100" onClick={() => handleCollegeSelect(college)}>
                ...
                <div className="flex gap-5 mb-5">
                  ...
                  <div className="flex-grow">
                    <input type="number" name="CurrentExperience" value={updatedCandidate.CurrentExperience} onChange={handleChange} id="CurrentExperience" min="1" max="15" autoComplete="given-name" class="border-b border-gray-300 focus:border-black focus:outline-none mb-5 w-full" />
