...
const Position_Form = ({ onClose, candidate1, rounds, isOpen, onOutsideClick }) => {
  ...
  const handleSubmit = async (_id, e) => {
    ...
    try {
      const response = await axios.put(`http://localhost:3000/position/${_id}`, data);
      setFormData({
        title: "",
        companyname: "",
        jobdescription: "",
  ...
  return (
    <>
      <div className="fixed inset-0 bg-black bg-opacity-15" style={{ zIndex: 999 }}>
        <div className="fixed inset-y-0 right-0 w-1/2 bg-white shadow-lg transition-transform duration-5000 transform 'translate-x-0' : 'translate-x-full'" style={{ zIndex: 999 }}>
              ...
              <form className="group" onSubmit={(e) => handleSubmit(formData._id, e)}>
                ...
                <div className="mt-5 text-sm">
                  <div className="mb-5 flex justify-between">
                    ...
                    <button
                      type="button"
                      className={`flex items-center justify-center text-sm bg-blue-500 text-white py-1 rounded w-28 ${roundEntries.length >= 5 ? 'cursor-not-allowed bg-blue-200' : ''}`}
